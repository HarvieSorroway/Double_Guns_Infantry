...
//	dbus_infomation.speed_wz=-0.8f;
	...
	/*ÔÆÌ¨½Ç¶ÈÒ£¿ØÆ÷¿ØÖÆ*/
	dbus_infomation.gimbal_yaw=RC_CtrlData.rc.ch2*24/DBUS_RESOLUTION;
	...
	if(RC_CtrlData.rc.s1==1)
	{
		...
		dbus_infomation.chassis_mode=gyro_mode;
	}
	...
	{
		dbus_infomation.shoot_state=shoot_start_sign;
		dbus_infomation.shoot_one=shoot_on;
	}
	else if(RC_CtrlData.rc.s2==3)
	{
		dbus_infomation.shoot_state=shoot_start_sign;
		dbus_infomation.shoot_one=shoot_off;
	}
	else if(RC_CtrlData.rc.s2==2)
	{
		dbus_infomation.shoot_state=shoot_stop_sign;
		dbus_infomation.shoot_one=shoot_off;
	}
}
