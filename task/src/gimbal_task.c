...
void gimbal_task(void *pvParameters)
{
	...
	for(;;)
	{
		...
		//pid test function
		m2006_position_pid_FollowChassis[0].Kp = FloatArray_GimbalCoefficients_position[0];
		m2006_position_pid_FollowChassis[0].Ki = FloatArray_GimbalCoefficients_position[1];
		m2006_position_pid_FollowChassis[0].Kd = FloatArray_GimbalCoefficients_position[2];
		
		m2006_speed_pid_FollowChassis[0].Kp = FloatArray_GimbalCoefficients_speed[0];
		m2006_speed_pid_FollowChassis[0].Ki = FloatArray_GimbalCoefficients_speed[1];
		m2006_speed_pid_FollowChassis[0].Kd = FloatArray_GimbalCoefficients_speed[2];
//		
